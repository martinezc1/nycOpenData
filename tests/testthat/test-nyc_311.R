# tests/testthat/test-nyc_311_robust.R

test_that("nyc_311 returns a tibble, respects limits, and supports date/range/filters", {
  vcr::use_cassette("nyc_311_robust", {
    # Base call
    base <- nyc_311(limit = 2)
    ...
    # date branch
    a <- nyc_311(date = "2025-03-01", limit = 2)
    expect_s3_class(a, "tbl_df")
    expect_true(nrow(a) >= 0 && nrow(a) <= 2)

    # from/to range branch
    b <- nyc_311(from = "2025-03-01", to = "2025-03-02", limit = 2)
    expect_s3_class(b, "tbl_df")
    expect_true(nrow(b) >= 0 && nrow(b) <= 2)

    # from only
    c <- nyc_311(from = "2025-03-01", limit = 2)
    expect_s3_class(c, "tbl_df")
    expect_true(nrow(c) >= 0 && nrow(c) <= 2)

    # to only
    d <- nyc_311(to = "2025-03-02", limit = 2)
    expect_s3_class(d, "tbl_df")
    expect_true(nrow(d) >= 0 && nrow(d) <= 2)

    # filters (equality)
    e <- nyc_311(limit = 2, filters = list(agency = "NYPD"))
    expect_s3_class(e, "tbl_df")
    expect_true(nrow(e) >= 0 && nrow(e) <= 2)

    # filters (IN ...)
    f <- nyc_311(limit = 2, filters = list(agency = c("NYPD", "DOT")))
    expect_s3_class(f, "tbl_df")
    expect_true(nrow(f) >= 0 && nrow(f) <= 2)
  })
})

test_that("nyc_311 input validation errors (date XOR from/to, bad dates, limit/filters/timeout)", {
  # date XOR from/to guard
  expect_error(
    nyc_311(date = "2025-03-01", from = "2025-03-01"),
  ...
  expect_error(
    nyc_311(date = "not-a-date", limit = 2),
    "standard unambiguous format|coercible to Date"
  )
  expect_error(
    nyc_311(from = "nope", limit = 2),
    "standard unambiguous format|coercible to Date"
  )
  expect_error(
    nyc_311(to = "nope", limit = 2),
    "standard unambiguous format|coercible to Date"
  )

  # limit validation
  expect_error(nyc_311(limit = "a string"), "`limit` must be")
  expect_error(nyc_311(limit = NA), "`limit` must be")
  expect_error(nyc_311(limit = -1), "between 0 and Inf")
  expect_error(nyc_311(limit = 1.2), "integer")

  # filters validation
  expect_error(nyc_311(filters = "not a list"), "`filters` must be")
  expect_error(nyc_311(filters = list("NYPD")), "named")
  expect_error(nyc_311(filters = list(agency = character(0))), "cannot be empty")
  expect_error(nyc_311(filters = list(agency = NA_character_)), "cannot contain NA")

  # timeout validation
  expect_error(nyc_311(timeout_sec = 0), "`timeout_sec` must be > 0")
  expect_error(nyc_311(timeout_sec = "fast"), "`timeout_sec` must be")
})
