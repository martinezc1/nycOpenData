...
By using the `nyc_pull_dataset` function, we can gather the most recent statistics from Local Law 18 and filter based upon any of the columns inside the dataset. 
...
Let's now take an example of the first 3 returned rows with less than 5 employees. The `nyc_pull_dataset` function can filter based on any of the columns in the dataset. To filter, we add `filters = list()` and put whatever filters we would like inside. From our `names` call before, we know that there is a column called "number_of_employees" which we can use to accomplish this.
...
Success! From calling the **nyc_locallaw18_payreport** from  `nyc_pull_dataset` we see there are only 3 rows of data, and from the `distinct()` call we see the only featured "row groups" are those with less than 5 employees. 

One of the strongest qualities this function has is its ability to filter based on of multiple columns. Let's put everything together and get a dataset of the last *15* pieces of data from the Local law 18 Pay Report bracket for "Technology & Innovation" with "less than 5 employees" and "female employees."
...
This graph shows us not only *how many* rows there are in the bracket of Technology & Innovation agencies with fewer than 5 employees (female employees only), but *what* the ethnicities of the workers there are.  
