...
``` r
small_sample <- nyc_pull_dataset("nyc_locallaw18_payreport", limit = 3)
small_sample
#> # A tibble: 3 × 11
...
lessthan5_locallaw18payreport <- nyc_pull_dataset("nyc_locallaw18_payreport", limit = 3, filters = list(number_of_employees = "<5"))
...
Success! From calling the **nyc_locallaw18_payreport** from
`nyc_pull_dataset` we see there are only 3 rows of data, and from the
