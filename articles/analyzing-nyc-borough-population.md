...
The `nyc_borough_population()` function provides access to historical
census data and population projections from 1950 to 2040, allowing one
to analyze demographic trends across Brooklyn, Bronx, Manhattan, Queens,
and Staten Island.
...
``` r
small_sample <- nyc_pull_dataset("nyc_borough_population", limit = 5)
small_sample
#> # A tibble: 5 × 22
...
``` r
population_data <- nyc_pull_dataset("nyc_borough_population")
...
``` r
brooklyn_pop <- nyc_pull_dataset("nyc_borough_population", filters = list(borough = "Brooklyn"))
...
The `nyc_borough_population()` function provides easy access to
demographic data for New York City spanning from 1950-2040. This enables
analysis of long-term population trends, comparisons across boroughs,
and exploration of projected future changes.
