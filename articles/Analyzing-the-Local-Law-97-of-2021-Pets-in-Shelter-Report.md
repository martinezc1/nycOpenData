...
`5nux-zfmw()` function, we can retrieve the most recent quarterly
shelter pet data and explore trends in pet ownership across time using
...
names(small_sample)
#>  [1] "date_year"                      "total_applicants"              
#>  [3] "had_pet"                        "client_borough_of_entry_bronx" 
#>  [5] "client_borough_of_entry"        "client_borough_of_entry_1"     
#>  [7] "clients_who_would_have_entered" "clients_choosing_to_forego"    
#>  [9] "clients_who_intend_to_regain"   "number_of_dogs"                
#> [11] "number_of_cats"                 "number_of_birds"               
#> [13] "number_of_small_mammals"        "number_of_reptiles_small"      
#> [15] "number_of_fish"                 "did_not_report_pet_type"       
#> [17] "pet_s_placed_with_foster"       "pet_s_surrendered_to_animal"   
#> [19] "pet_s_placed_with_family"       "pet_s_placed_with_friend"      
#> [21] "pet_s_placed_with_other"        "pet_s_placed_with_non_profit"  
#> [23] "date_quarter"                   "chose_to_forego_shelter_for"
```

Fantastic! We successfully pulled the
`Local Law 97 of 2021-Pets in Shelter Report` data from the NYC Open
Data Portal using the `5nux-zfmw()` function.

Let’s now look at an example using the most recent 3 quarters of data.
The `5nux-zfmw()` function allows users to filter based on any of the
columns in the dataset. To apply filters, we add `filters = list()` and
specify the desired conditions. From our earlier inspection of the
column names, we know that there are columns such as `date_year` and
`date_quarter` that can be used to filter the data by time period.
...
#>       <dbl> <chr>                   <dbl>   <dbl>                         <dbl>
#> 1      2024 Qtr 1                   11651       3                             2
#> 2      2024 Qtr 2                   12274       4                             4
#> 3      2024 Qtr 3                   11901       2                             2
